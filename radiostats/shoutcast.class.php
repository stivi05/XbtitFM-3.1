...
//		    fputs($fp, "GET /admin.cgi?pass=".$this->passwd."&mode=viewxml HTTP/1.0\r\n");
		    fputs($fp, "GET /admin.cgi?pass=".$this->passwd."&mode=viewxml&sid=1 HTTP/1.0\r\n");
...
$this->_xml .= fgets($fp, 512);
		    ...
		    if (stristr($this->_xml, "HTTP/1.1 200 OK") == true) {
		    	// <-H> Thanks to Blaster for this fix.. trim();
    			$this->_xml = trim(substr($this->_xml, 59));
			} else {
				$this->_error = "Bad Login";
//				$this->_error = $this->_xml;
				return(0);
			}
