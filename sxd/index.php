...
class Sypex_Dumper {
	...
	function connect($host = null, $port = null, $user = null, $pass = null){
		...
		if($GLOBALS['conn']=mysqli_connect($this->CFG['my_host'] . ($this->CFG['my_host']{0} != ':' ? ":{$this->CFG['my_port']}" : ''),  $this->CFG['my_user'], $this->CFG['my_pass'])) {
			do_sqlquery('SET NAMES utf8') or sxd_my_error();
			define('V_MYSQL', sxd_ver2int(mysqli_get_server_info($GLOBALS['conn'])));
		}
		else {
			define('V_MYSQL', 0); 
			$this->error = "sxd.actions.tab_connects();alert('{" . mysqli_real_escape_string($GLOBALS['conn'], mysqli_error($GLOBALS['conn'])) . "');";
		}	
		$this->try = false;
	...
	function ajax($req){
		...
		switch($req['act']){
			...
			case 'exit': 
				setcookie('sxd', '', 0);
				$res = "this.location.href = '" . mysqli_real_escape_string($GLOBALS['conn'],$this->CFG['exitURL']) . "'"; 
				break;
		}	
		echo $res;
	}
	...
	function deleteDB($name){
		$r = do_sqlquery('DROP DATABASE `' . mysqli_real_escape_string($GLOBALS['conn'],$name) . '`') or sxd_my_error();
		if($r){
        	echo "sxd.clearOpt('db');sxd.addOpt(" . sxd_php2json(array('db' => $this->getDBList())) . ");sxd.combos.services_db.select(0,'-');";
		}
        else
        	echo "alert('" . mysqli_real_escape_string($GLOBALS['conn'],mysqli_error($GLOBALS['conn'])) . "')";
	}
	function cfg2js($cfg){
		foreach($cfg AS $k => $v){
			$cfg[$k] = mysqli_real_escape_string($GLOBALS['conn'],$v);
		}
		return $cfg;
	}
	function addDb($req){
        $r = do_sqlquery('CREATE DATABASE `' . mysqli_real_escape_string($GLOBALS['conn'],$req['name']) . '`' . (V_MYSQL > 40100 ? "CHARACTER SET {$req['charset']} COLLATE {$req['collate']}" : ''));
        if($r)
	...
	function runServices($job) {
		...
		if(isset($serv[$job['type']])) {
			mysqli_select_db($GLOBALS['conn'], $job['db']);
			$filter = $object = array();
			$this->createFilters($job['obj'], $filter, $object);
			$r = do_sqlquery('SHOW TABLE STATUS') or sxd_my_error();
			if (!$r) return;
			$tables = array();
			...
			$r = do_sqlquery($sql) or sxd_my_error();
			if (!$r) return;
			$res = array();
	...
	function runRestoreJob($continue = false){
		...
		mysqli_select_db($GLOBALS['conn'],$this->JOB['db']);
		...
		if($this->JOB['strategy'] > 0){
			$r = do_sqlquery("SHOW TABLES") or sxd_my_error();
			while($item = mysqli_fetch_row($r)){
		...
		//$enable_index = array();
		if(V_MYSQL > 40014) {
			do_sqlquery("SET UNIQUE_CHECKS=0");
			do_sqlquery("SET FOREIGN_KEY_CHECKS=0");
			if(V_MYSQL > 40101) do_sqlquery("SET SQL_MODE='NO_AUTO_VALUE_ON_ZERO'");
			if(V_MYSQL > 40111) do_sqlquery("SET SQL_NOTES=0");
		}
		$log_sql = false;
		$fields = '';
		$time_old = time();
		...
		while($q = sxd_read_sql($this->fh_tmp, $seek, $ei)){	
			...
			switch($q{0}){
				...
				case 'I':
					if (preg_match('/^INSERT( INTO `(.+?)`) VALUES/', $q, $m)) {
						...
						//do_sqlquery("LOCK TABLES `{$tab}` WRITE") or die (mysqli_error($GLOBALS['conn']));
						do_sqlquery("ALTER TABLE `{$tab}` DISABLE KEYS") or sxd_my_error();
						if(!empty($this->JOB['autoinc'])) do_sqlquery("ALTER TABLE `{$tab}` AUTO_INCREMENT = 1") or sxd_my_error();
						$ex = 1;
					}
					break;
				case 'C':
					...
					if (preg_match("/\#\t(TC|TD|VI|PR|FU|TR|EV)`(.+?)`(([^_]+?)_.+?)?$/", $q, $m)) {
						...
						if($m[1] == 'TC') {
							...
							if($tc && ($this->JOB['strategy'] == 0 || isset($tab_exists[$m[2]]))) {
								do_sqlquery("{$tc} `{$m[2]}`") or sxd_my_error();
							}
						}
						elseif($m[1] == 'TD'){
							$ei = 1;
							...
							$this->rtl[7] = 0;
							$this->rtl[8] = 0;
							do_sqlquery("DROP {$types[$m[1]]} IF EXISTS `{$m[2]}`") or sxd_my_error();
			...
			if($ex) {
				...
				if($c){
					$i = mysqli_affected_rows($GLOBALS['conn']);
					$this->rtl[3] = ftell($this->fh_tmp) - $seek;
					$this->rtl[7] += $i;
					$this->rtl[10] += $i;
					fseek($this->fh_rtl, 0);
		...
		foreach($this->JOB['todo']['TA'] AS $tab){
			...
			do_sqlquery("ALTER TABLE `{$tab[1]}` ENABLE KEYS") or sxd_my_error();
			$this->rtl[1] = time();
			$this->rtl[5] = $tab[1];
			fseek($this->fh_rtl, 0);
			fwrite($this->fh_rtl, implode("\t", $this->rtl));
	...
	function addBackupJob($job) {
		...
		foreach($queries AS $query){
			...
			if($t == 'TA' && (!empty($object['TC']) || !empty($filter['TC']))) {}
			elseif(empty($object[$t]) && empty($filter[$t])) continue;
			$r = do_sqlquery('SHOW ' . $query[0]) or sxd_my_error();
			if (!$r) continue;
			$todo[$t] = array();
			$header[$t] = array();
		...
		if (V_MYSQL > 50014 && (!empty($object['VI']) || !empty($filter['VI']))) {
			...
			$r = do_sqlquery("SELECT table_name, view_definition /*!50121 , collation_connection */ FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = '{$this->JOB['db']}'") or sxd_my_error();
		...
		$this->JOB['todo'] = $todo;
		$this->saveJob($this->JOB['job'], $this->JOB);
		$fcache = implode('|', array('#SXD20', V_SXD, V_MYSQL, V_PHP, date('Y.m.d H:i:s'), $this->JOB['db'], $this->JOB['charset'], $tabs, $rows, mysqli_real_escape_string($GLOBALS['conn'],$this->JOB['comment']))) . "\n";
	...
	function runBackupJob($continue = false){
		...
		do_sqlquery("SET SQL_QUOTE_SHOW_CREATE = 1");
		$types = array('VI' => 'View', 'PR' => 'Procedure', 'FU' => 'Function', 'TR' => 'Trigger', 'EV' => 'Event');
		$fcache = '';
		$writes = 0;
		
		do_sqlquery("SET SESSION character_set_results = '" . ($this->JOB['charset'] ? $this->JOB['charset'] : 'binary') ."'") or sxd_my_error();
		$time_old = time();
		$exit_time = $time_old + $this->CFG['time_web'] - 1;
				...
				switch($n[0]){
					...
					case 'TA':
						...
						if ($n[0] == 'TC' || $this->rtl[7] == 0){
							...
							$r = do_sqlquery("SHOW CREATE TABLE `{$n[1]}`") or sxd_my_error();
						...
						$notNum = array();
						$r = do_sqlquery("SHOW COLUMNS FROM `{$n[1]}`") or sxd_my_error();
			            $fields = 0;
			            ...
			            $z = 0;
			            // Достаем данные
			            $r = mysqli_query($GLOBALS['conn'],"SELECT {$no_cache}* FROM `{$n[1]}`{$from}");
			            while($row = mysqli_fetch_row($r)) {
							...
							for($k = 0; $k < $fields; $k++){
								...
								elseif($notNum[$k]) {$row[$k] =  '\'' . mysqli_real_escape_string($GLOBALS['conn'],$row[$k]) . '\'';}
							}
							$fcache .= '(' . implode(',', $row) . "),\n";
							$this->rtl[7]++;  
							$this->rtl[10]++;
					...
					default:
						if(V_MYSQL < 50121 && $n[0] == 'TR'){
							...
							$r = do_sqlquery("SELECT * FROM `INFORMATION_SCHEMA`.`TRIGGERS` WHERE `TRIGGER_SCHEMA` = '{$this->JOB['db']}' AND `TRIGGER_NAME` = '{$n[1]}'") or sxd_my_error();
							$item = mysqli_fetch_assoc($r);
							...
							$r = do_sqlquery("SHOW CREATE {$types[$n[0]]} `{$n[1]}`") or sxd_my_error();
							$item = mysqli_fetch_assoc($r);
	...
	function setNames($collation){
		...
		if($this->rtl[6] != $collation) {
			do_sqlquery('SET NAMES \'' . preg_replace('/^(\w+?)_/', '\\1\' COLLATE \'\\1_', $collation) . '\'') or sxd_my_error();
	...
	function getDBList(){
		...
		if ($this->CFG['my_db']) {
			...
			foreach($tmp AS $d){
				$d = trim($d);
				$items[] = $qq . mysqli_real_escape_string($GLOBALS['conn'],$d) . $qq;
				$dbs[$d] = "{$d} (0)";
			}
		}
		else{
			$result = do_sqlquery("SHOW DATABASES") or sxd_my_error();
    		while($item = mysqli_fetch_row($result)){
    			if($item[0] == 'information_schema' || $item[0] == 'mysql') continue;
    			$items[] = $qq . mysqli_real_escape_string($GLOBALS['conn'],$item[0]) . $qq;
    			$dbs[$item[0]] = "{$item[0]} (0)";
    		}	
		}
		if(V_MYSQL < 50000){
			foreach($items AS $item){
    			$tables = do_sqlquery("SHOW TABLES FROM `{$item}`") or sxd_my_error();
    			if ($tables) {
			...
			$result = do_sqlquery("SELECT `table_schema`, COUNT(*) FROM `information_schema`.`tables` {$where} GROUP BY `table_schema`") or sxd_my_error();
			while($item = mysqli_fetch_row($result)){
    			if($item[0] == 'information_schema' || $item[0] == 'mysql') continue; 
	...
	function getCharsetList(){
		...
		if(V_MYSQL > 40101) {
    		...
    		$r = do_sqlquery("SHOW CHARACTER SET") or sxd_my_error(); 
    		if ($r) {
    			while($item = mysqli_fetch_assoc($r)){
	...
	function getCollationList(){
		...
		if(V_MYSQL > 40101) {
    		...
    		$r = do_sqlquery("SHOW COLLATION") or sxd_my_error(); 
    		if ($r) {
    			while($item = mysqli_fetch_assoc($r)){
	...
	function getObjects($tree, $db_name){
		mysqli_select_db($GLOBALS['conn'],$db_name);
		// Достаем таблицы
		$r = do_sqlquery('SHOW TABLE STATUS') or sxd_my_error();
		...
		if($r){
			...
			if (V_MYSQL > 50014 && $tree != 'services') {
				...
				for($i = 0, $l = count($shows); $i < $l; $i++){
					$r = do_sqlquery('SHOW ' . $shows[$i]) or sxd_my_error();
	...
	function formatTree($tree, &$objects){
		...
		foreach($objects AS $type => $o){
			...
			if($type == 'TA') {
				...
				$obj .= "[{$row},0,'" . mysqli_real_escape_string($GLOBALS['conn'],$info[$type][0]) . "',1,1,1],";
				$row++;
				foreach($o['*'] AS $value){
					if(is_string($value)){
						if(count($o[$value]) > 1)	{
							...
							for($i = 0, $l = count($o[$value]); $i < $l; $i++){
								...
								$obj .= "[{$row},{$pid},'" . mysqli_real_escape_string($GLOBALS['conn'],$o[$value][$i][0]) . "',2,{$checked},{$o[$value][$i][2]}],";
								$row++;
							}
						}
						else {
				...
				$obj .= "[{$row},0,'" . mysqli_real_escape_string($GLOBALS['conn'],$info[$type][0]) . "',{$info[$type][1]},1,1],";
				$pid = $row++;
				$info[$type][1]++;
				for($i = 0, $l = count($o); $i < $l; $i++){
					$o[$i] = mysqli_real_escape_string($GLOBALS['conn'],$o[$i]);
...
function sxd_error_handler($errno, $errmsg, $filename, $linenum, $vars){
    ...
    if($errno == 8 || $errno == 1024) {
    	if (!$SXD->fh_log && !$SXD->fh_rtl) echo isset($_POST['ajax']) ? "alert('" . mysqli_real_escape_string($GLOBALS['conn'],$str) . "');" : $str;
    	else {
...
function sxd_my_error(){
	trigger_error(mysqli_error($GLOBALS['conn']), E_USER_ERROR);	
}
