<script type="text/javascript" src="jscript/tabber.js"></script>
<link rel="stylesheet" href="jscript/tabber.css" TYPE="text/css" MEDIA="screen">
<div class="tabber">

     <div class="tabbertab">
	  <h2><tag:lang.SETUP_MSG /></h2>
<form name="welcome" method="post" action="<tag:wmact />">
<table width="100%" cellpadding="0" cellspacing="0" border="0">
<tr>
<td class="header"><tag:lang.SUBJECT /></td><td class="lista"><input type="text" name="SUB" value="<tag:wmset.fm_welcome_sub />" size="60"></td></tr>
<tr>
<td class="lista" colspan="2">
<tag:wmbb />
</td>
</tr>
<tr>
<td class="header" colspan="2" style="text-align:center"><input class="btn" type="submit" value="<tag:lang.SUBMIT />"></td>
</tr>
</table>
</form>
</div>
<div class="tabbertab">
	  <h2><tag:lang.PREVIEW_MSG /></h2>
<table width="50%" align="center" cellpadding="0" cellspacing="0" border="0">
<tr>
<td class="header"><tag:lang.SUBJECT />:&nbsp;<tag:wmprevsub /></td>
</tr>
<td class="lista" style="text-align:center"><tag:wmprev /></td>
</tr>
</table>
</div>
</div>
