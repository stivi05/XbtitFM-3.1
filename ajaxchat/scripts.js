...
function Pophistory(){newWin=window.open('index.php?page=allshout','shouthistory','height=500,width=490,resizable=yes,scrollbars=yes');if(window.focus){newWin.focus()}}
function PopCmd(){newWin=window.open('cmdshout.php','Commands','height=300,width=490,resizable=yes,scrollbars=yes,top=200,left=300');if (window.focus) {newWin.focus()}}
function PopPshout(from,name,pchat){newWin=window.open('index.php?page=Pshout&fromid='+from+'&toid='+name+'&pchat='+pchat,'PrivateShout','height=400,width=600,resizable=yes,scrollbars=yes,top=300,left=300');if(window.focus){newWin.focus()}}
...
function handlehHttpReceiveChat(){if(httpReceiveChat.readyState==4){var results=document.getElementById("outputList");results.innerHTML=httpReceiveChat.responseText;setTimeout('receiveChatText();',30000);}}
function sendComment(){if(httpSendChat.readyState==4||httpSendChat.readyState==0){currentChatText=encodeURIComponent(document.forms['chatForm'].elements['chatbarText'].value);if(currentChatText!=''){currentName=encodeURIComponent(document.forms['chatForm'].elements['name'].value);currentUid=document.forms['chatForm'].elements['uid'].value;param='n='+currentName+'&c='+currentChatText+'&u='+currentUid;httpSendChat.open("POST",SendChaturl,true);httpSendChat.setRequestHeader('Content-Type','application/x-www-form-urlencoded');httpSendChat.onreadystatechange=handlehHttpSendChat;httpSendChat.send(param);document.forms['chatForm'].elements['chatbarText'].value='';}}else{setTimeout('sendComment();',1000);}}
