...
function mysql_init() {
    ...
    if ($dbpass!='') {
        $link = @mysqli_connect($dbhost, $dbuser, $dbpass) or die("Cannot connect to database!"); 
    } else {
        $link = @mysqli_connect($dbhost, $dbuser) or die("Cannot connect to database!"); 
    }
    mysqli_select_db($link,$database) or die("Cannot select database!");
    return $link;
}
...
if ($userid!="") {
    ...
    $result = mysqli_query($mi,$query) or die("Could not select data!");
    $s = mysqli_fetch_array($result,MYSQLI_NUM);
    $counter = $s[0];
